...
// Robert Alessio  - Phi/Delta    - ratio of phi and delta  at different k and z
  ...
  double poissonratio_at_k_and_z(struct background * pba,
                          ...
                          double k,
                          double z,
                          //double * pk_tot, /* pointer to a single number (must be already allocated) */   this was the cause of the segfault
    ...
    fprintf(stdout,"%d\n", psp->ln_k[i]);
     ...
     /*
      * I optimized the finding of the index in the ln_k vector
      */
     for (i = 0; i < psp->ln_k_size; i ++)
     {
               ...
               struct spectra * psp, double z)
{
   ...
   {  
     ...
     {   
       ...
       psp->PhiOverPsi[index_tau*psp->ln_k_size + index_k] = log(phi/psi);
     } 
   }
   return _SUCCESS_;
}
...
double PhiPsiratio_at_k_and_z(struct background * pba,
                          struct spectra * psp,
                          struct perturbs *ppt,
                          double k,
                          double z,  
     ...
     class_call(ratioPhiPsi(pba,ppt,psp,z),
               psp->error_message,
               psp->error_message);
     double diff2;
     double k_used2;
    ...
    slope = (psp->PhiOverPsi[i_k+1] - psp->PhiOverPsi[i_k])/dk; 	
    variable = psp->PhiOverPsi[i_k] + (log(k)- (psp->ln_k[i_k]) )*slope;
    free(psp->PhiOverPsi);
    return exp(variable);
}
               ...
               struct spectra * psp, double z)
{
   ...
   class_alloc(psp->growthfac, sizeof(double)*psp->ln_tau_size*psp->ln_k_size,psp->error_message);
   for (index_tau=0 ; index_tau < psp->ln_tau_size; index_tau++) 
   {  
     for (index_k=0; index_k<psp->ln_k_size; index_k++) 
     {   
       ...
       psp->growthfac[index_tau*psp->ln_k_size + index_k] = dplus;
     } 
   }
   return _SUCCESS_;
}
  
double matterdens_at_z_and_k(struct background * pba,
                          struct spectra * psp,
                          struct perturbs *ppt,
                          double k,
                          double z,
                          ...
                          )
{
     ...
     class_call(matterdens_at_z(pba,ppt,psp,z),
               psp->error_message,
               psp->error_message);
     double diff2;
     double k_used2;
    ...
    double dk;
    dk = (psp->ln_k[i_k+1]- psp->ln_k[i_k]); 
    slope = (psp->growthfac[i_k+1] - psp->growthfac[i_k])/dk; 	
    variable = psp->growthfac[i_k] + (log(k)- (psp->ln_k[i_k]) )*slope;
    free(psp->growthfac);
    return variable;
}
