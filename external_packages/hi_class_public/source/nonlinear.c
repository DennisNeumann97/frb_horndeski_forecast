...
int nonlinear_init(
  ...
  else if ((pnl->method == nl_halofit) || ((pnl->method == nl_HMcode))) { //fprintf(stdout, "nonlinearmethod %d \n", pnl->method);
      ...
      if (pnl->method == nl_halofit) {
        if (print_warning == _FALSE_) {
          ...
          if (halofit_found_k_max == _TRUE_) {
            ...
            /*
							for (index_k=0; index_k<pnl->k_size; index_k++) {
								fprintf(stdout,"ecco %e  %e  %e\n",pnl->k[index_k],pk_l[index_k],pk_nl[index_k]);
							 }
								fprintf(stdout,"\n\n");
            */
              for (index_k=0; index_k<pnl->k_size; index_k++) { //fprintf(stdout, "sqrt %e \n", sqrt(pk_nl[index_pk][index_k]/pk_l[index_pk][index_k]));
        ...
        if (print_warning == _FALSE_) {
          ...
          if (halofit_found_k_max == _TRUE_) {
						...
						/*
							...
							if (index_tau == pnl->tau_size-1) fprintf(stdout,"ecco2 %e  %e  %e\n",pnl->k_extra[index_k],pk_l[index_k],pk_l_cb[index_k]);
							}
							if (index_tau == pnl->tau_size-1) fprintf(stdout,"\n\n");
						*/
            
              for (index_k=0; index_k<pnl->k_size; index_k++) {//fprintf(stdout,"ecc sqrt %e \n", pk_l[index_pk][index_k]);//pk_nl[index_pk][index_k]);///pk_l[index_pk][index_k]));
                pnl->nl_corr_density[index_pk][index_tau * pnl->k_size + index_k] = sqrt(pk_nl[index_pk][index_k]/pk_l[index_pk][index_k]);
              }
          }
          else {
...
int nonlinear_hmcode_fill_growtab(
  ...
  for (index_scalefactor=0;index_scalefactor<ng;index_scalefactor++){
    ...
    //for (index_scalefactor=0;index_scalefactor<ng;index_scalefactor++){
      //pnl->growtable[index_scalefactor] = pnl->growtable[index_scalefactor]/ pnl->growtable[ng-1];      			
    }   for (index_scalefactor=0;index_scalefactor<ng;index_scalefactor++){
      pnl->growtable[index_scalefactor] = pnl->growtable[index_scalefactor]/ pnl->growtable[ng-1];      			
...
int nonlinear_hmcode(
  ...
  /* data fields */ 
  double * pvecback;  double * pvecback2;
  double * conc;
  double * mass;
  double * sigma_r;
	double * sigmaf_r;
  ...
  class_alloc(pvecback,pba->bg_size*sizeof(double),pnl->error_message); class_alloc(pvecback2,pba->bg_size*sizeof(double),pnl->error_message);
  ...
  anorm    = 1./(2*pow(_PI_,2));  
   //Alessio
  //class_call(background_at_tau(pba,tau,pba->long_info,pba->inter_normal,&last_index,pvecback2),pba->error_message, pnl->error_message);
  double tau_finale;           
  class_call(background_tau_of_z(pba, 0, &tau_finale),pba->error_message, pnl->error_message);
  class_call(background_at_tau(pba,tau_finale,pba->long_info,pba->inter_normal,&last_index,pvecback2),pba->error_message,pnl->error_message);
  double growth2 = pvecback2[pba->index_bg_D];           
  ...
  growth = pvecback[pba->index_bg_D];  growth /= growth2;      			
