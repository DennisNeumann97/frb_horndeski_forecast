...
int spectra_pk_nl_at_z(
  ...
  if (psp->ln_tau_size == 1) {
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      output_tot[index_k] = psp->ln_pk_nl[index_k];
            //fprintf(stdout, "single power at indexk= %d \t %e \n",index_k,output_tot[index_k]);
  ...
  /** - --> (b) if several values of tau have been stored, use interpolation routine to get spectra at correct redshift */
    ...
    if(ln_tau < psp->ln_tau_nl[0]){
      ...
      //fprintf(stdout, "multiple power at indexk= %d \t %e \n",index_k,output_tot[index_k]);
  ...
  if (mode == linear) {
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      output_tot[index_k] = exp(output_tot[index_k]);
      fprintf(stdout, "power at indexk= %d \t %e \n",index_k,output_tot[index_k]);
      if(pba->has_ncdm) output_cb_tot[index_k] = exp(output_cb_tot[index_k]);
    }
  }
...
// Robert Alessio  - Phi/Delta    - simple ratio  of phi and delta
/*int ratio(struct background * pba,
               struct perturbs * ppt,
               struct spectra * psp, double z)
{
   ...
   index_md = psp->index_md_scalars; //scalar perturbations only

   double tau;
 	class_call(background_tau_of_z(pba,z,&tau),
	               pba->error_message,
	               psp->error_message); //convert z to tau
   double phi; // local variable
   double delta;
   
   // loop over initial conditions and perturb phi and delta_m
   for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++) 
   { 
      class_call(perturb_sources_at_tau(ppt,index_md,index_ic1,ppt->index_tp_phi,tau,
             ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_phi]),
		       ppt->error_message, psp->error_message);
      class_call(perturb_sources_at_tau(ppt,index_md,index_ic1,ppt->index_tp_delta_m,tau,
             ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_delta_m]),
		       ppt->error_message, psp->error_message); 
   }   
   
   // allocate vector for the ratio of phi and delta
   class_alloc(psp->poissonratio, sizeof(double)*psp->ln_tau_size*psp->ln_k_size,psp->error_message);
   
   // tau loop only consists out of one entry (index_tau = 0)
   for (index_tau=0 ; index_tau < psp->ln_tau_size; index_tau++) 
   {  
     // loops over all k
     for (index_k=0; index_k<psp->ln_k_size; index_k++) 
     {   
       phi = 0.;
       delta = 0.;
       // loops over initial conditions
       for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++) 
       { 
          phi += ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_phi][index_tau*ppt->k_size[index_md]+index_k];
         delta += ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_delta_m][index_tau*ppt->k_size[index_md]+index_k];
       }
       psp->poissonratio[index_tau*psp->ln_k_size + index_k] = log(-phi/delta);
     } 
   }
   return _SUCCESS_;
}*/
...
int ratio(struct background * pba,
               struct perturbs * ppt,
               struct spectra * psp, double z, double * output_tot)
{
  ...
  // class_alloc(psp->poissonratio, sizeof(double)*psp->ln_tau_size*psp->ln_k_size,psp->error_message);
   ...
   {  
     ...
     for (index_k=0; index_k<psp->ln_k_size; index_k++) 
     {
      ...
      output_tot[index_tau*psp->ln_k_size + index_k] = log(-phi/delta);
     } 
   }
   return _SUCCESS_;
}
...
// Robert Alessio  - Phi/Delta    - ratio of phi and delta  at different k and z
/*double poissonratio_at_k_and_z(struct background * pba,
                          struct spectra * psp,
                          struct perturbs *ppt,
                          double k,
                          double z,
     ...
     class_call(ratio(pba,ppt,psp,z),
               psp->error_message,
               psp->error_message);
    // double diff;
     double k_used;
    ...
    fprintf(stdout,"dopo\n");
     ...
     {
         if((exp(psp->ln_k[i+1]) > k ) && (exp(psp->ln_k[i]) < k ))
       {
         i_k = i;
	 break;
       }
    ...
    double dk2;
    fprintf(stdout,"%d\n", i_k);
    fprintf(stdout,"ciaonissimo\n");
    dk2 = (psp->ln_k[i_k+1]- psp->ln_k[i_k]);
    fprintf(stdout,"ciao6\n");
    slope = (psp->poissonratio[i_k+1] - psp->poissonratio[i_k])/dk2;
    
    variable = psp->poissonratio[i_k] + (log(k)- (psp->ln_k[i_k]) )*slope;
    free(psp->poissonratio);
    fprintf(stdout,"%d\n", variable);
    fprintf(stdout,"%d\n", exp(variable));
    double res = -exp(variable);
    //return -exp(variable);
    return res;
}*/
                          
	//int poissonratio_at_k_and_z(struct background * pba,        I chaged this function so that it returns directly the ratio now. It's not the best, but 
	// I could not find easily something else to avoid the segmentation fault
int poissonratio_at_k_and_z(struct background * pba,
                          struct spectra * psp,
                          struct perturbs *ppt,
                          double k,
                          double z,
    ...
    // double diff;
     double k_used;
     double variable;
     double * ratio_at_z = NULL;
    // diff = 1e10;
     int i,i_k;
    
     class_alloc(ratio_at_z, sizeof(double)*psp->ln_tau_size*psp->ln_k_size,psp->error_message);
          class_call(ratio(pba,ppt,psp,z,ratio_at_z),
               psp->error_message,
               psp->error_message);
     for (i = 0; i < psp->ln_k_size; i ++)
     {
    ...
    double slope; 
    double dk;
    dk = (psp->ln_k[i_k+1]- psp->ln_k[i_k]);
    
    slope = (ratio_at_z[i_k+1] - ratio_at_z[i_k])/dk;
    
    	  
    	
     variable = ratio_at_z[i_k] + (log(k)- (psp->ln_k[i_k]) )*slope;
     free(ratio_at_z);
    *output = -exp(variable);
    //fprintf(stdout,"%d\n", k);
...
// Robert Alessio  - Phi/Psi    - simple ratio  of phi and psi

int ratioPhiPsi(struct background * pba,
               struct perturbs * ppt,
               struct spectra * psp, double z, double * output_tot)
{
   ...
   { 
      class_call(perturb_sources_at_tau(ppt,index_md,index_ic1,ppt->index_tp_phi,tau,
             ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_phi]),
		       ppt->error_message, psp->error_message);
      class_call(perturb_sources_at_tau(ppt,index_md,index_ic1,ppt->index_tp_psi,tau,
             ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_psi]),
		       ppt->error_message, psp->error_message); 
   }   
 //  class_alloc(psp->PhiOverPsi, sizeof(double)*psp->ln_tau_size*psp->ln_k_size,psp->error_message);
   for (index_tau=0 ; index_tau < psp->ln_tau_size; index_tau++) 
   {  
     for (index_k=0; index_k<psp->ln_k_size; index_k++) 
     {   
       ...
       { 
          phi += ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_phi][index_tau*ppt->k_size[index_md]+index_k];
          psi += ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_psi][index_tau*ppt->k_size[index_md]+index_k];	   	
       }
       output_tot[index_tau*psp->ln_k_size + index_k] = log(phi/psi);
     } 
   }
   return _SUCCESS_;
}
...
// Robert Alessio  - Phi/Psi   - ratio of potentials at different k and z
  
                          
int PhiPsiratio_at_k_and_z(struct background * pba,
                          struct spectra * psp,
                          struct perturbs *ppt,
                          double k,
                          double z,  
                          double * pk_ic,   /* array of argument pk_ic[index_ic1_ic2] (must be already allocated only if several initial conditions) */		  double * result
                          ) {
     ...
     double k_used2;
     double variable;
     double * PhiPsi_z = NULL;
     int i,i_k;
     class_alloc(PhiPsi_z, sizeof(double)*psp->ln_tau_size*psp->ln_k_size,psp->error_message);
     class_call(ratioPhiPsi(pba,ppt,psp,z,PhiPsi_z),
               psp->error_message,
               psp->error_message);
     for (i = 0; i < psp->ln_k_size; i ++)
     {
    ...
    double dk;
    dk = (psp->ln_k[i_k+1]- psp->ln_k[i_k]); 
    slope = (PhiPsi_z[i_k+1] - PhiPsi_z[i_k])/dk; 	
    variable = PhiPsi_z[i_k] + (log(k)- (psp->ln_k[i_k]) )*slope;
    free(PhiPsi_z);
    *result = exp(variable);
    return _SUCCESS_;
}
...
int matterdens_at_z(struct background * pba,
               struct perturbs * ppt,
               struct spectra * psp, double z, double * output_tot)
{
   int index_md;
   ...
   {  
     ...
     {   
       ...
       for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++) 
       { 
          dplus += ppt->sources[index_md][index_ic1*ppt->tp_size[index_md] + ppt->index_tp_delta_m][index_tau*ppt->k_size[index_md]+index_k];
       }
      output_tot[index_tau*psp->ln_k_size + index_k] = dplus;
     } 
   }
   return _SUCCESS_;
}
  
int matterdens_at_z_and_k(struct background * pba,
                          struct spectra * psp,
                          struct perturbs *ppt,
                          double k,
                          double z,
                          ...
                          )
{
     ...
     double variable;
     int i,i_k;
     double * matter_z = NULL;
     class_alloc(matter_z, sizeof(double)*psp->ln_tau_size*psp->ln_k_size,psp->error_message);
     class_call(matterdens_at_z(pba,ppt,psp,z,matter_z),
               psp->error_message,
               psp->error_message);
     for (i = 0; i < psp->ln_k_size; i ++)
     {
    ...
    double dk;
    dk = (psp->ln_k[i_k+1]- psp->ln_k[i_k]); 
    slope = (matter_z[i_k+1] - matter_z[i_k])/dk; 	
    variable = matter_z[i_k] + (log(k)- (psp->ln_k[i_k]) )*slope;
    free(matter_z);
    *result = variable;
    return _SUCCESS_;
}
	           ...
	           /*
 * Alessio and Robert, given the Hubble function at some tau
 */


double hubble_at_z(struct background *pba, struct spectra *psp, double z)
{
  double tau;
  double * pvecback;
  ...
  class_call(background_tau_of_z(pba,z,&tau),pba->error_message, psp->error_message); //convert z to tau
  ...
  class_call(background_at_tau(pba,
               ...
               pba->error_message,
               pba->error_message);  
  double result = pvecback[pba->index_bg_H];
  free(pvecback);
  return result;
}

double hubbleprime_at_z(struct background *pba, struct spectra *psp, double z)
{
  double tau;
  double * pvecback;
  int first_index_back;
  ...
  class_call(background_at_tau(pba,
               ...
               pba->error_message,
               pba->error_message);
  double result = pvecback[pba->index_bg_H_prime];
  free(pvecback);  
   return result;
}
