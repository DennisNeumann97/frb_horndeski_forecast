...
cdef class Class:
    ...
    def scale_independent_growth_factor(self, z):
        ...
        """
        cdef double tau
        cdef double tau_finale
        cdef int last_index #junk
        cdef double * pvecback
        cdef double * pvecback2

        pvecback = <double*> calloc(self.ba.bg_size,sizeof(double))
        pvecback2 = <double*> calloc(self.ba.bg_size,sizeof(double))
        ...
        if background_tau_of_z(&self.ba,0.,&tau_finale)==_FAILURE_:
            raise CosmoSevereError(self.ba.error_message)

        if background_at_tau(&self.ba,tau_finale,self.ba.long_info,self.ba.inter_normal,&last_index,pvecback2)==_FAILURE_:
            raise CosmoSevereError(self.ba.error_message)

        Dnorm = pvecback2[self.ba.index_bg_D]
    ...
    def get_poissonratio_screened(self, double k, double z, double screening_scale, double a):
        cdef result 
        result = 1. + (a * self.get_poissonratio(k, z) - 1.)*np.exp(-(k*k/2./screening_scale/screening_scale))
        return result

    def get_poissonratio(self, double k, double z):
        cdef double dummy
        cdef double result
        poissonratio_at_k_and_z(&self.ba, &self.sp, &self.pt, k, z, &dummy, &result)
        #result = poissonratio_at_k_and_z(&self.ba, &self.sp, &self.pt, k, z, &dummy)
        return result 
    
    def get_phipsiratio_screened(self, double k, double z, double screening_scale):
        cdef result
        result = 1. + (self.get_phipsiratio(k, z) - 1.)*np.exp(-(k*k/2./screening_scale/screening_scale))
        return result
    
    def get_phipsiratio(self, double k, double z):
        cdef double dummy
        cdef double result
        PhiPsiratio_at_k_and_z(&self.ba, &self.sp, &self.pt, k, z, &dummy, &result)
        return result
    
    def get_Hubble(self, double z):
        cdef double result
        result = hubble_at_z(&self.ba,&self.sp, z)
        return result
    
    def get_Growth(self, double k, double z):
        cdef double result
        cdef double result1
        cdef double dummy
        matterdens_at_z_and_k(&self.ba,&self.sp,&self.pt,k,z,&dummy,&result1)
        matterdens_at_z_and_k(&self.ba,&self.sp,&self.pt,k,0.,&dummy,&result)
        return result1/result

    def screened_growth(self, double k, double z, double screening_scale):
        cdef double result
        cdef double scale_ind
        scale_ind = self.scale_independent_growth_factor(z)
        dtilde = self.get_Growth(k, z) - scale_ind
        dtilde*= np.exp(-(k**2.)/(2.*screening_scale*screening_scale))
        return scale_ind + dtilde
